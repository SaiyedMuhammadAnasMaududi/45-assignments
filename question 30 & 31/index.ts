...
for(let i=0;i<=5;i++){ 
     if(userarr[i]==="Admin"){console.log(`HEllo! ${userarr[i]} would you like to see a status report?`)}
     else{console.log(`Hello ! ${userarr[i]} thank you for logging in.`)}
}
...
if(userarr.length!=0){
     console.log(`The list of usernames is not empty.`)
}else{console.log(` As the list is now empty ;We need to find some Users !`)}
