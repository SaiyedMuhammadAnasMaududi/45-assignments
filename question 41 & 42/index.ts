...
function make_album(artistname:string,albumtitle:string,tracks?:number){
  let album:{name: string;
   Albumtitle: string;
   tracks?: number}={name:artistname,Albumtitle:albumtitle,
...
function show_magician(copyarrray?:string[][]| string[],magiciian?:string[]){
  
if(copyarrray===copyarray){
  console.log(copyarray)}

else if(copyarrray==magician){ magician=magician.map(magicia=>`${magicia}The Great`)
  console.log(magician)}}
// }}
// // if(magician){
